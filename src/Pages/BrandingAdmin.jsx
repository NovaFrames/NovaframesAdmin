...
import { collection, getDoc, setDoc ,addDoc, doc, updateDoc, deleteDoc } from "firebase/firestore";
import {
  Palette,
  Brush,
  BarChart3,
...
const BrandingAdmin = () => {
  ...
  const handleDeleteService = async (id) => {
    try {
      const updatedServices = brandingData.services.filter(s => s.id !== id);
      await updateDoc(doc(db, "branding", "brandingData"), {
        services: updatedServices
      });
      setBrandingData({ ...brandingData, services: updatedServices });
  ...
  const handleDeleteProcess = async (id) => {
    try {
      const updatedProcess = brandingData.process.filter(p => p.id !== id);
      await updateDoc(doc(db, "branding", "brandingData"), {
        process: updatedProcess
      });
      setBrandingData({ ...brandingData, process: updatedProcess });
  ...
  const handleDeleteFaq = async (id) => {
    try {
      const updatedFaqs = brandingData.faqs.filter(f => f.id !== id);
      await updateDoc(doc(db, "branding", "brandingData"), {
        faqs: updatedFaqs
      });
      setBrandingData({ ...brandingData, faqs: updatedFaqs });
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="mt-2">
                Selected icon: {serviceIcons[editingService.icon]}
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Title</label>
        ...
        <div className="bg-gray-50 p-4 rounded-lg mb-6">
          ...
          <div className="space-y-4">
            <div>
              ...
              <input
                type="text"
                value={editingProcess.step}
                onChange={(e) => setEditingProcess({ ...editingProcess, step: e.target.value })}
                className="w-full p-2 border border-gray-300 rounded-md"
              />
            </div>
            <div>
